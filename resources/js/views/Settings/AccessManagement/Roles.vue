...
const headers = computed(() => [
    {title: t('settings.access-management.roles.data-table.default.headers.name'), key: 'name'},
    {title: t('settings.access-management.roles.data-table.default.headers.description'), key: 'description'},
    {title: t('settings.access-management.roles.data-table.default.headers.guardName'), key: 'guard_name'},
    {
        title: t('settings.access-management.roles.data-table.default.headers.permissions'),
        key: 'permissions',
        sortable: false
    },
    {title: t('settings.access-management.roles.data-table.default.headers.actions'), key: 'actions', sortable: false},
]);
...
const formTitle = computed(() => {
    return editedItem.value?.id
        ? t('settings.access-management.roles.data-table.default.dialog.form-title-edit')
        : t('settings.access-management.roles.data-table.default.dialog.form-title-new');
});
...
const deleteItemConfirm = async () => {
    try {
        await axios.delete(`/roles/${editedItem.value.id}`);
...
const formatPayload = () => ({
    ...editedItem.value,
    permissions: editedItem.value.permissions.map(permission => ({id: permission})),
});
...
const save = async () => {
    try {
        ...
        backendErrorTimer.value = setTimeout(() => {
            backendErrors.value = {};
            rolesForm?.value?.resetValidation();
...
<template>
    <v-data-table-server
            ...
            :items-per-page-text="t('settings.access-management.roles.data-table.default.perPageText')"
    >
        <template v-slot:top>
            <v-toolbar flat>
                <v-toolbar-title
                        class="text-primary"
                >
                    {{ t('settings.access-management.roles.toolbar-title') }}
                </v-toolbar-title>
                <v-spacer/>
                ...
                <v-text-field
                        ...
                        color="primary"
                        base-color="primary"
                        :label="t('settings.access-management.roles.searchTextFieldLabel')"
                        density="compact"
                        hide-details
                        outlined
                ></v-text-field>
                ...
                <v-btn
                        ...
                        :text="searchMode === 'simple' ? t('settings.access-management.roles.advancedSearchBtnText') : t('settings.access-management.roles.simpleSearchBtnText')"
                                    ...
                                    <v-row>
                                        <v-col cols="12">
                                            <v-text-field
                                                    ...
                                                    :rules="lengthRules(3, 255, t('settings.access-management.roles.data-table.default.dialog.nameTextFieldLabel'))"
                                                    :error-messages="backendErrors.name || []"
                                                    :label="t('settings.access-management.roles.data-table.default.dialog.nameTextFieldLabel')"
                                                    outlined
                                                    density="compact"
                                                    clearable
                                                    color="primary"
                                    ...
                                    <v-row>
                                        <v-col cols="12">
                                            <v-textarea
                                                    ...
                                                    :rules="lengthRules(3, 255, t('settings.access-management.roles.data-table.default.dialog.descriptionTextareaLabel'), false)"
                                                    :error-messages="backendErrors.description || []"
                                                    :label="t('settings.access-management.roles.data-table.default.dialog.descriptionTextareaLabel')"
                                                    no-resize
                                                    :maxlength="maxLength"
                                                    counter
                                                    density="comfortable"
                                    ...
                                    <v-row>
                                        <v-col cols="12">
                                            <v-select
                                                    ...
                                                    v-on:keyup.enter="save"
                                                    :items="guardNames"
                                                    :rules="lengthRules(3, 255,t('settings.access-management.roles.data-table.default.dialog.guardNameSelectLabel'))"
                                                    :error-messages="backendErrors.guard_name || []"
                                                    :label="t('settings.access-management.roles.data-table.default.dialog.guardNameSelectLabel')"
                                                    outlined
                                                    density="compact"
                                                    clearable
                                                    color="primary"
                                    ...
                                    <v-row>
                                        <v-col cols="12">
                                            <v-autocomplete
                                                    ...
                                                    item-title="name"
                                                    item-value="id"
                                                    :rules="requiredRules(t('settings.access-management.roles.data-table.default.dialog.autocomplete.permissionsLabel'))"
                                                    :error-messages="backendErrors.permissions || []"
                                                    :label="t('settings.access-management.roles.data-table.default.dialog.autocomplete.permissionsLabel')"
                                                    :no-data-text="t('settings.access-management.roles.data-table.default.dialog.autocomplete.noDataText')"
                                                    chips
                                                    closable-chips
                                                    multiple
                                                    auto-select-first
                ...
                <v-row>
                    <v-col cols="5">
                        <v-text-field
                                ...
                                :label="t('settings.access-management.roles.advancedSearch.nameTextFieldLabel')"
                                density="compact"
                                clearable
                                color="primary"
                                base-color="primary"
                    ...
                    <v-col cols="5">
                        <v-text-field
                                ...
                                v-on:keyup.enter="fetchItems"
                                :label="t('settings.access-management.roles.advancedSearch.descriptionTextFieldLabel')"
                                density="compact"
                                clearable
                                color="primary"
                                base-color="primary"
                    ...
                    <v-col cols="2">
                        <v-select
                                ...
                                :items="guardNames"
                                :label="t('settings.access-management.roles.advancedSearch.guardNameSelectLabel')"
                                outlined
                                density="compact"
                                clearable
                                color="primary"
                ...
                <v-row>
                    <v-col cols="6">
                        <v-text-field
                                ...
                                v-on:keyup.enter="fetchItems"
                                :label="t('settings.access-management.roles.advancedSearch.permissionSelectLabel')"
                                density="compact"
                                clearable
                                color="primary"
                                base-color="primary"
        ...
        <template v-slot:no-data>
            <div class="text-center">
                <p>{{ t('settings.access-management.roles.data-table.default.noDataMessage') }}</p>
                <v-btn
                        @click="searchMode === 'simple' ? resetSearch() : resetAdvancedSearch()"
                        append-icon="mdi-refresh"
                        color="primary"
                >
                    {{ t('settings.access-management.roles.data-table.default.reset') }}
                </v-btn>
            </div>
        </template>
    </v-data-table-server>
